...
- 보라·핑크 그라데이션 “AI 대시보드” 팔레트 유지 — YouTube Studio / GitHub 다크 등 **무료로 열리는 실제 화면**으로 토큰을 다시 잡음
...
- **배경:** `#0f1015` 유지하되, 보라 안개·멀티 그라데이션 CTA는 줄인다. 액센트는 **한 색 + 황금키워드 앰버**.
- **레퍼런스 툴:** [`docs/design-tools.md`](design-tools.md) — **무료 우선.** YouTube Studio·GitHub 다크·uiverse. Mobbin은 유료라 쓰지 않음.
- **타이포:** Pretendard / system-ui. Ledger의 Fraunces는 넣지 않음. 제목만 `clamp()`.
- **카피:** 버튼은 기능이 드러나게. 모든 버튼에 이모지를 붙이지 않음.
- **모션:** hover 1px / active 0.98 / 칩 선택만. 네온 보더·무한 pulse 남발 금지.
- **황금키워드 설명 한 줄:** “웹에서 키워드를 긁지 않고, 지금 급등 중인 영상 제목에서 검색 문구를 뽑습니다.”
...
| **조사 도구 5종** | [`docs/mockups/work-five.html`](mockups/work-five.html) |
| UX 재조사 | [`docs/ux-research.md`](ux-research.md) |
